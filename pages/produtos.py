...
CAMINHO_FICHA = _c["ficha"]

COLUNAS = ['cod_prod', 'nome', 'desc', 'tipo', 'categoria', 'p_venda', 'p_custo', 'margem', 'tipo_venda', 'insumo_direto']
...
os.makedirs(os.path.dirname(CAMINHO_INSUMOS), exist_ok=True)
os.makedirs(os.path.dirname(CAMINHO_FICHA), exist_ok=True)
...
def carregar_backup(caminho):
    with open(caminho, 'rb') as f:
        ...
        for col in ['cod_prod', 'nome', 'desc', 'tipo', 'categoria']:
            if col in df.columns:
                df[col] = df[col].astype(str)
        return df
...
if 'produtos' not in st.session_state:
    st.session_state.produtos = carregar_df(CAMINHO_PRODUTOS, COLUNAS)
    if st.session_state.produtos.empty:
        st.session_state.produtos = pd.DataFrame(columns=COLUNAS)

if 'insumos' not in st.session_state:
    st.session_state.insumos = carregar_df(CAMINHO_INSUMOS, COLUNAS_INSUMOS)
...
if 'ficha_tecnica' not in st.session_state:
    st.session_state.ficha_tecnica = carregar_df(CAMINHO_FICHA, COLUNAS_FICHA)
...
# aba1, aba2, aba3, aba4, aba5 = st.tabs(["Cadastrar", "Menu", "Ficha Técnica", 'Insumos', "Restaurar"])
                ...
                if not df.empty:
                    ...
                    if not produto.empty:
                        ...
                        st.session_state.form_data = {
                            ...
                            'p_venda': float(row['p_venda']) if pd.notna(row['p_venda']) else 0.0,
                            'p_custo': float(row['p_custo']) if pd.notna(row['p_custo']) else 0.01,
                            'tipo_venda': str(row['tipo_venda']) if pd.notna(row['tipo_venda']) else 'menu',
                            'insumo_direto': str(row['insumo_direto']) if pd.notna(row['insumo_direto']) else ''
                        }
    ...
    with st.form("form_produto"):
        ...
        if submitted:
            ...
            if not erros:
                ...
                if salvar_df(st.session_state.produtos, CAMINHO_PRODUTOS):
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    with open(os.path.join(CAMINHO_BACKUP, f"produtos_{timestamp}.pkl"), 'wb') as f:
                        pickle.dump(st.session_state.produtos.to_dict(), f)
...
with aba2:
    ...
    if not df.empty:
        # ORDENAÇÃO SEGURA: converte para numérico, ignora textos
...
with aba3:
    ...
    if df_produtos.empty:
        ...
        if not ficha_produto.empty and not st.session_state.insumos.empty:
            ...
            if custo_total_produto > 0:
                ...
                if not idx_prod.empty:
                    ...
                    salvar_df(st.session_state.produtos, CAMINHO_PRODUTOS)
        ...
        # ============================================================
        # ADICIONAR / EDITAR / REMOVER INSUMOS
        # ============================================================
                        ...
                        with col4:
                            if st.button("🗑️", key=f"remove_ficha_{row['id_insumo']}_{cod_prod_selecionado}", help="Remover este insumo"):
                                ...
                                if not idx_remover.empty:
                                    ...
                                    if salvar_df(st.session_state.ficha_tecnica, CAMINHO_FICHA):
        ...
        with col_btn1:
            if st.form_submit_button("➕ Adicionar", use_container_width=True):
                if novo_nome:
                    ...
                    if salvar_df(st.session_state.insumos, CAMINHO_INSUMOS):
...
# with aba5:
#     st.subheader("Restaurar Versões")

#     pkl_files = sorted([f for f in os.listdir(CAMINHO_BACKUP) if f.endswith('.pkl')], reverse=True)

#     if pkl_files:
#         tabela_backups = []
#         for idx, arquivo in enumerate(pkl_files, 1):
#             caminho = os.path.join(CAMINHO_BACKUP, arquivo)
#             try:
#                 df = carregar_backup(caminho)
#                 timestamp_str = arquivo.replace('produtos_', '').replace('.pkl', '')
#                 try:
#                     data_hora = datetime.strptime(timestamp_str, "%Y%m%d_%H%M%S").strftime("%d/%m/%Y %H:%M:%S")
#                 except:
#                     data_hora = timestamp_str
#                 tabela_backups.append({
#                     'Codigo': f"BKP-{idx:04d}",
#                     'Data/Hora': data_hora,
#                     'Qtd Produtos': len(df),
#                     'Arquivo': arquivo
#                 })
...
#         df_backups = pd.DataFrame(tabela_backups)
#         st.dataframe(df_backups[['Codigo', 'Data/Hora', 'Qtd Produtos']], use_container_width=True, hide_index=True)

#         codigos = [f"BKP-{i+1:04d}" for i in range(len(pkl_files))]
#         codigo_selecionado = st.selectbox("Selecione o backup para carregar:", codigos)

#         if st.button("Carregar Backup", use_container_width=True):
#             idx = int(codigo_selecionado.split('-')[1]) - 1
#             caminho = os.path.join(CAMINHO_BACKUP, pkl_files[idx])
#             st.session_state.backup_confirm = True
#             st.session_state.backup_file = caminho
#             st.session_state.backup_name = pkl_files[idx]
#             st.rerun()

#         if st.session_state.get('backup_confirm', False):
#             df_preview = carregar_backup(st.session_state.backup_file)
#             st.warning(f"""
#             ATENCAO - CONFIRMACAO NECESSARIA

#             Substituir produtos atuais pelo backup:
#             - Backup: {st.session_state.backup_name}
#             - Produtos no backup: {len(df_preview)}
#             - Produtos atuais: {len(st.session_state.produtos)}
#             """)
#             col_ok, col_cancel = st.columns(2)
#             with col_ok:
#                 if st.button("SIM, CARREGAR", use_container_width=True):
#                     try:
#                         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#                         with open(os.path.join(CAMINHO_BACKUP, f"antes_restaurar_{timestamp}.pkl"), 'wb') as f:
#                             pickle.dump(st.session_state.produtos.to_dict(), f)

#                         df_restaurado = carregar_backup(st.session_state.backup_file)
#                         st.session_state.produtos = df_restaurado.copy()
#                         if salvar_df(st.session_state.produtos, CAMINHO_PRODUTOS):
#                             st.session_state.backup_confirm = False
#                             st.success(f"Backup restaurado! {len(df_restaurado)} produtos.")
#                             time.sleep(1)
#                             st.rerun()
#                     except Exception as e:
...
#         st.divider()
#         st.subheader("Deletar Backups")

#         codigos_deletar = st.multiselect("Selecione backups para deletar:", codigos)
#         if codigos_deletar and st.button("DELETAR SELECIONADOS", use_container_width=True):
#             for cod in codigos_deletar:
#                 idx = int(cod.split('-')[1]) - 1
#                 caminho = os.path.join(CAMINHO_BACKUP, pkl_files[idx])
#                 if os.path.exists(caminho):
#                     os.remove(caminho)
#             st.success(f"{len(codigos_deletar)} backup(s) deletados!")
#             time.sleep(1)
#             st.rerun()
#     else:
#         st.info("Nenhum backup disponivel")

#     st.divider()
#     if st.button("Salvar Backup Atual", use_container_width=True):
#         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#         with open(os.path.join(CAMINHO_BACKUP, f"produtos_{timestamp}.pkl"), 'wb') as f:
#             pickle.dump(st.session_state.produtos.to_dict(), f)
#         if salvar_df(st.session_state.produtos, CAMINHO_PRODUTOS):
#             st.success(f"Backup salvo! {len(st.session_state.produtos)} produtos.")
#             time.sleep(1)
#             st.rerun()